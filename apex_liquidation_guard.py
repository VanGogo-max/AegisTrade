# apex_liquidation_guard.py
"""
Apex Liquidation Guard (FINAL)

Responsibility:
- Evaluate distance to liquidation price
- Block execution when safety buffer is violated
- Pure risk layer, no execution, no signing
"""

from typing import Dict, Any


class ApexLiquidationRisk(Exception):
    pass


class ApexLiquidationGuard:
    ...
    def evaluate(self, position: Dict[str, Any], mark_price: float) -> None:
        ...
        if buffer < self.min_buffer_ratio:
            raise ApexLiquidationRisk(
                f"Apex liquidation risk too high. "
