# kwenta_emergency_shutdown.py
"""
Kwenta Emergency Shutdown (FINAL)

Responsibility:
- Global hard kill-switch for Kwenta trading
- Immediately blocks:
    - new orders
    - position increases
    - leverage changes
...
class KwentaEmergencyShutdown(Exception):
    pass


class KwentaEmergencyController:
    def __init__(self):
        self._active = False
        self._reason = None
    ...
    def activate(self, reason: str) -> None:
        ...
        raise KwentaEmergencyShutdown(f"EMERGENCY SHUTDOWN: {reason}")
    ...
    def assert_trading_allowed(self, action: str) -> None:
        ...
        if action.upper() not in ("CLOSE", "REDUCE"):
            raise KwentaEmergencyShutdown(
                f"Kwenta emergency active since {self._activated_at}, "
